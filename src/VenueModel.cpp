...
bool VenueModel::dynamicRoles() const
{
    return m_dynamicRoles;
}

void VenueModel::setDynamicRoles(bool dynamicRoles)
{
...
QJSValue VenueModel::attachedProperties() const
{
    return m_attachedProperties;
}

bool VenueModel::addRole(const QString &role)
{
...
QJSValue VenueModel::clone(QQmlEngine *engine, const QJSValue &src) const
{
    if (!src.isObject())
        return src;
...
bool VenueModel::extractRoles(const QJSValue &item,
                                 const QString &prefix = QString())
{
    ...
    if (prefix.isNull() && (item.isString() || item.isNumber() || item.isDate())) {
        if (addRole("modelData"))
            rolesAdded = true;
            std::cout << "added roles: model data: " << std::endl;
    } else {
        ...
        while (it.next()) {
            ...
            {
                rolesAdded = true;
                std::cout << "added role " << n.toStdString() << std::endl;
            }
            QJSValue v = it.value();
...
void VenueModel::emitCountChanged()
{
    emit countChanged(rowCount());
}

int VenueModel::addItem(const QJSValue &item)
{
    int row = -1;
...
void VenueModel::add(const QJSValue &item)
{
    m_lock->lockForWrite();
...
void VenueModel::remove(const QJSValue &item)
{
    int index;
    {
        QString key;
...
QJSValue VenueModel::at(int row) const
{
...
QJSValue VenueModel::get(const QJSValue &id) const
{
...
QJSValue VenueModel::asArray(bool deepCopy) const
{
...
QModelIndex VenueModel::index(int row, int column, const QModelIndex &) const
{
    QReadLocker readLock(m_lock);
    if (row >= 0 && row < m_keys.count()) {
...
QModelIndex VenueModel::parent(const QModelIndex &) const
{
    return QModelIndex();
}
...
QString VenueModel::getRole(int role) const
{
    role -= BASE_ROLE;
...
int VenueModel::getRole(const QString &role) const
{
...
void VenueModel::setAttachedProperties(QJSValue attachedProperties)
{
...
bool VenueModel::setData(const QModelIndex &, const QVariant &, int)
{
    return false;
}
